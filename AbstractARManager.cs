...
namespace maxstAR
{
	...
	public abstract class AbstractARManager : MonoBehaviour
	{
		...
		protected void Init()
		{
            int screenScale = (int)Screen.dpi/100;
            arCamera = GetComponent<Camera>();
            ARManagerTargetObject aRManagerTargetObject = GetComponent<ARManagerTargetObject>();
            if(aRManagerTargetObject != null) {
                targetImage = aRManagerTargetObject.targetImageUI;
                if(arCamera != null) {
                    float width = targetImage.rectTransform.rect.width;
                    float height = targetImage.rectTransform.rect.height;
                    renderTexture = new RenderTexture((int)(width * screenScale), (int)(height * screenScale), 24);
                    arCamera.targetTexture = renderTexture;
                    targetImage.material.mainTexture = renderTexture;
            ...
            if(targetImage != null) {
                MaxstAR.OnSurfaceChanged((int)targetImage.rectTransform.rect.width, (int)targetImage.rectTransform.rect.height);
            } else {
		...
		void InitInternal()
		{
			...
			{
                if (targetImage != null) {
                    screenWidth = (int)targetImage.rectTransform.rect.width;
                    screenHeight = (int)targetImage.rectTransform.rect.height;
        ...
        void OnPreRender()
		{
            if (targetImage != null)
            {
                if (screenWidth != (int)targetImage.rectTransform.rect.width || screenHeight != (int)targetImage.rectTransform.rect.height)
                {
                    screenWidth = (int)targetImage.rectTransform.rect.width;
                    screenHeight = (int)targetImage.rectTransform.rect.height;
                    if (arCamera.targetTexture != null)
                    {
                        arCamera.targetTexture.Release();
                    }
                    int screenScale = (int)Screen.dpi / 100;
                    float width = targetImage.rectTransform.rect.width;
                    float height = targetImage.rectTransform.rect.height;
		...
		private void TransformBackgroundPlane(Camera camera, Transform planeTransform)
        {
            ...
            float widthRatio = (float)screenWidth / cameraWidth;
			float heightRatio = (float)screenHeight / cameraHeight;
			...
			float viewWidth = (float)frustumWidth / screenWidth;
